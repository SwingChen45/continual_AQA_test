...
def train_epoch(
    ...
    for batch_idx, (feat_vst, feat_flow, feat_stgcn, scores, action_id) in enumerate(dataloader):
        batch_size = action_id.shape[0]

        if combined_exemplar is not None:
            select_vst, select_flow, select_stgcn, select_score, select_action_id, select_id = \
                utils.random_select_exemplar(combined_exemplar, batch_size, count)
            feat_vst = torch.cat([feat_vst, select_vst], dim=0)
            feat_flow = torch.cat([feat_flow, select_flow], dim=0)
        ...
        diff_loss = 0.0
        if combined_exemplar is not None:
            aug_helper_vst, aug_helper_flow, aug_helper_stgcn, aug_helper_score, aug_helper_action_id, aug_helper_select_id = \
                utils.random_select_exemplar(combined_exemplar, args.num_helpers, count)
            aug_helper_vst = aug_helper_vst.cuda()
            aug_helper_flow = aug_helper_flow.cuda()
            ...
            aug_helper_feat, _ = run_jrg(
                jrg_pre, aug_helper_vst, aug_helper_flow, aug_helper_stgcn,
                save_graph=args.save_graph, seen_tasks=seen_tasks, args=args
            )
...
def run_exp(args, task_list, action2task, classes_name, exemplar_set):
    ...
    for t, task in enumerate(task_list):
        ...
        if args.replay:
            packed_list = []
            for data_idx in range(len(purely_task_data[3])):
                packed_list.append([
                    purely_task_data[0][data_idx],
                    purely_task_data[1][data_idx],
                    purely_task_data[2][data_idx],
                    purely_task_data[3][data_idx],
                ])
