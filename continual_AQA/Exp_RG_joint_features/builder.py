...
def _load_rg_joint_split(data_root, joint_root, action_name, split='train'):
    ...
    if any(x is None for x in [vst_path, flow_path, label_path, joint_path, joint_label_path]):
        raise FileNotFoundError(
            'Incomplete RG joint files for action={}, split={}: '
    ...
    n = len(scores)
    if not (len(vst) == len(flow) == len(joint) == n):
        raise ValueError(
            'Size mismatch in {}-{}: vst={}, flow={}, joint={}, labels={}'.format(
                action_name, split, len(vst), len(flow), len(joint), n
            )
        )
...
def load_data(data_root, set_id, args, exemplar_set=None):
    ...
    train_pack = _load_rg_joint_split(data_root, args.joint_root, action_name, split='train')
    test_pack = _load_rg_joint_split(data_root, args.joint_root, action_name, split='test')
...
def get_parser():
    ...
    parser.add_argument('--model_name', type=str, default='RG_JOINT_FEATURES_ASS')
