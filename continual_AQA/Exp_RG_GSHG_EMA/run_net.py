...
from models.GSHG_HyperGCN import run_jrg, init_e_graph, update_domain_graph
    ...
    for batch_idx, (feat_joint, scores, action_id) in enumerate(dataloader):
        ...
        if combined_exemplar is not None:
            ...
            if current_old_half < feat_pre.shape[0]:
                ...
                if args.diff_loss:
                    score_diff = aug_score - scores[current_old_half:]
                    combined_feature = torch.cat((aug_feat, feat[current_old_half:]), dim=-1)
...
def run_exp(args, task_list, action2task, classes_name, exemplar_set):
    ...
    for i in range(len(task_list)):
        _, _, _, loader_test, _ = builder.load_data(
            data_root=None, set_id=i, args=args, exemplar_set=None
        )
    ...
    for t, task in enumerate(task_list):
        ...
        dataset_train, dataset_test, loader_train, loader_test, purely_task_data = builder.load_data(
            data_root=None, set_id=task, args=args, exemplar_set=exemplar_set
        )
        max_domain_batches = args.domain_update_batches if args.domain_update_batches > 0 else None
        update_domain_graph(
            jrg,
            loader_train,
            device='cuda',
            rho=args.ema_rho,
            max_batches=max_domain_batches,
        )
        log_print(
            logger,
            f'EMA quality domain graph updated: rho={args.ema_rho}, '
            f'batches={"all" if max_domain_batches is None else max_domain_batches}'
        )
