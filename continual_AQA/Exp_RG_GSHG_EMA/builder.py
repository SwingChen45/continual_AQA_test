...
def build_moodel(args):
    model_ = ASS_GSHG(
        ...
        quality_dyn_weight=args.quality_dyn_weight,
    )
...
def get_parser():
    ...
    parser.add_argument('--ema-rho', type=float, default=0.9)
    parser.add_argument('--quality-dyn-weight', type=float, default=0.1)
    parser.add_argument('--domain-update-batches', type=int, default=0,
                        help='0 means using the full current-task train loader to update EMA domain graph')
    parser.add_argument('--gshg-graph-lr', type=float, default=0.001)
    parser.add_argument('--gshg-hyper-lr', type=float, default=0.0005)
    parser.add_argument('--model_name', type=str, default='RG_GSHG_EMA')
