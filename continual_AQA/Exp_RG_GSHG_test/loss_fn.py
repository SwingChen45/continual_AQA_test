...
def ge_graph_distill_(model_, model_pre_, seen_tasks=[],mse=None):
    graph_distill_loss = 0.0
    if hasattr(model_.module, 'gshg'):
        cur = model_.module.gshg
        pre = model_pre_.module.gshg
        graph_distill_loss += mse(pre.hyper_edge_weight_gen.detach(), cur.hyper_edge_weight_gen)
        for task in seen_tasks:
            if task == seen_tasks[-1]:
                continue
            graph_distill_loss += mse(
                pre.hyper_edge_weight_spec[task].detach(),
                cur.hyper_edge_weight_spec[task]
            )
        return graph_distill_loss
    ...
    spatial_graphs_pre = (1-model_pre_.module.alpha) * spatial_graphs_pre + model_pre_.module.alpha * general_spatial_graphs_pre
    temporal_graphs_pre = (1-model_pre_.module.alpha) * temporal_graphs_pre + model_pre_.module.alpha * general_temporal_graphs_pre
...
if __name__ == '__main__':
    ...
    model_ = ASS_GSHG(patch_size=256, seg_num=72, joint_num=18, out_dim=1,save_graph=True, mode = '', G_E_graph=True, alpha=0.5)
