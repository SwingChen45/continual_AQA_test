...
class GSHGHyperGC(nn.Module):
    ...
    def __init__(
        ...
        self.hyper_edge_weight_gen = nn.Parameter(torch.ones(num_subset, self.total_vertex))
        self.hyper_edge_weight_spec = nn.Parameter(torch.ones(task_num, num_subset, self.total_vertex))
    ...
    def forward(self, x):
        ...
        W_gen = W_dyn * self.hyper_edge_weight_gen.unsqueeze(0)
        ...
        for task_id in range(self.task_num):
            W_spec = W_dyn * self.hyper_edge_weight_spec[task_id].unsqueeze(0)
