...
def after_train(t, task, exemplar_set, packed_list, args, jrg, score_rgs, seen_tasks):
    ...
    packed_sorted_list = sorted(packed_list, key=lambda x: x[3])
    if t != 0:
        m_p = int(args.memory_size / t)
        reduce_exemplar_sets(m, m_p, exemplar_set)
    data2save = get_m_exemplar_simple(task, packed_sorted_list=packed_sorted_list, m=m, args=args,
                                      feature_extracor=jrg, rgs=score_rgs, seen_tasks=seen_tasks)
...
def get_features_score(jrg, rgs, loader, seen_tasks, args):
    ...
    for batch_idx, (feat_vst, feat_flow, feat_stgcn, scores, action_id, data_id) in enumerate(loader):
        feat_vst = feat_vst.cuda()
        feat_flow = feat_flow.cuda()
        feat_stgcn = feat_stgcn.cuda()
        scores = scores.float()
        action_id = action_id.cuda()

        feat, _ = run_jrg(jrg, feat_vst, feat_flow, feat_stgcn, save_graph=args.save_graph, seen_tasks=seen_tasks, args=args)
        if args.save_graph:
    ...
    if exemplar_set is not None:
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                ...
                train_score = torch.cat((train_score, torch.tensor(exemplar[3]).float().unsqueeze(0)), dim=0)
                train_action_name += [a]
    if len(train_action_name) == 0:
        return None
    return train_vst, train_flow, train_stgcn, train_score, train_action_name



def random_select_exemplar(combined_exemplar, b, count=0):
    train_vst, train_flow, train_stgcn, train_score, train_action_name = combined_exemplar
    np.random.seed(count)
    select_id = np.random.randint(0, train_score.shape[0], size=b)
    select_vst = train_vst[select_id]
    select_flow = train_flow[select_id]
    select_stgcn = train_stgcn[select_id]
    select_score = train_score[select_id]
    select_action_name = torch.Tensor(train_action_name)[select_id]
    return select_vst, select_flow, select_stgcn, select_score, select_action_name, select_id
