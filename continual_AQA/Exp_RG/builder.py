...
def _find_file(data_root, must_have_keywords):
    files = os.listdir(data_root)
    hits = []
    keywords = [k.lower() for k in must_have_keywords]
    for f in files:
        fl = f.lower()
        if all(k in fl for k in keywords):
            hits.append(os.path.join(data_root, f))
    if len(hits) == 0:
        raise FileNotFoundError(f'Cannot find file in {data_root} containing keywords: {must_have_keywords}')
    hits = sorted(hits)
    return hits[0]



def _resolve_action_name(set_id, args=None):
...
def _load_label_pkl(file_path):
    ...
    if isinstance(obj, (list, tuple)) and len(obj) == 2:
        sample_ids, scores = obj
    else:
        raise ValueError(f'Unexpected label pkl format in {file_path}. Expected (sample_ids, scores).')
...
def _load_rg_one_split(data_root, action_name, split='train'):
    aliases = ACTION_ALIASES[action_name]

    action_dir = os.path.join(data_root, action_name)
    if not os.path.isdir(action_dir):
        raise FileNotFoundError(f'Cannot find action directory: {action_dir}')
    ...
    for alias in aliases:
        ...
        try:
            ...
            pass
        try:
            if stgcn_path is None:
                stgcn_path = _find_file(action_dir, [alias, 'stgcn', split])
        except FileNotFoundError:
            pass
        try:
    ...
    if any(x is None for x in [vst_path, flow_path, stgcn_path, label_path]):
        raise FileNotFoundError(
            f'Incomplete files for action={action_name}, split={split}. '
            f'Looked inside: {action_dir}. '
            f'Found: vst={vst_path}, flow={flow_path}, stgcn={stgcn_path}, label={label_path}'
        )

    vst = np.load(vst_path).astype(np.float32)
    flow = np.load(flow_path).astype(np.float32)
    stgcn = np.load(stgcn_path).astype(np.float32)
    sample_ids, scores = _load_label_pkl(label_path)

    n = len(scores)
    if not (len(vst) == len(flow) == len(stgcn) == n):
        raise ValueError(
            f'Size mismatch in {action_name}-{split}: '
            f'vst={len(vst)}, flow={len(flow)}, stgcn={len(stgcn)}, labels={n}'
        )
...
def load_data(data_root, set_id, args, exemplar_set=None):
    action_name = _resolve_action_name(set_id, args)

    train_pack = _load_rg_one_split(data_root, action_name, split='train')
    test_pack = _load_rg_one_split(data_root, action_name, split='test')

    train_vst = train_pack['vst']
    train_flow = train_pack['flow']
    train_stgcn = train_pack['stgcn']
    train_scores = train_pack['scores'].copy()

    test_vst = test_pack['vst']
    test_flow = test_pack['flow']
    test_stgcn = test_pack['stgcn']
    ...
    train_stgcn_with_memory = torch.tensor(train_stgcn).float()
    ...
    if (exemplar_set is not None) and (args.dataset_mixup):
        ...
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                ...
                train_stgcn_with_memory = torch.cat((train_stgcn_with_memory, torch.tensor(exemplar[2]).float().unsqueeze(0)), dim=0)
    ...
    purely_task_data = (train_vst, train_flow, train_stgcn, train_scores)
...
def load_data_from_list(data_list=None, cls_label=0):
    ...
    if data_list is not None:
        for idx, exemplar in enumerate(data_list):
            ...
            train_stgcn = torch.cat((train_stgcn, torch.tensor(exemplar[2]).float().unsqueeze(0)), dim=0)
...
def build_exp():
    ...
    # Fixed task orders for 4-task continual experiments.
    task_list_bank = [
        [0, 1, 2, 3],
        [1, 3, 0, 2],
        [2, 0, 3, 1],
