...
class GCNLayer_LoRA(nn.Module):
    def __init__(self, in_feats, out_feats, rank, negative_slope=0.2):
        super(GCNLayer_LoRA, self).__init__()
        self.lin_src_a = nn.Linear(in_feats, rank, bias=False)
        self.lin_src_b = nn.Linear(rank, out_feats, bias=False)
        self.linear = nn.Sequential(self.lin_src_a, self.lin_src_b)
        self.leaky_relu = nn.LeakyReLU(negative_slope)
    ...
    def reset_parameters(self):
        """Reinitialize learnable parameters."""
        gain = nn.init.calculate_gain('leaky_relu', 0.2)
        nn.init.xavier_normal_(self.lin_src_a.weight, gain=gain)
        nn.init.xavier_normal_(self.lin_src_b.weight, gain=gain)
