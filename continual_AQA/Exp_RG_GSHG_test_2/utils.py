...
import torch
import builder
from models.GSHG_HyperGCN import run_jrg
...
def get_features_score(jrg, rgs, loader, seen_tasks, args):
    ...
    for batch_idx, (feat_joint, scores, action_id, data_id) in enumerate(loader):
        feat_joint = feat_joint.cuda()
        scores = scores.float()
        action_id = action_id.cuda()

        feat, _ = run_jrg(jrg, feat_joint, save_graph=args.save_graph, seen_tasks=seen_tasks, args=args)
        if args.save_graph:
