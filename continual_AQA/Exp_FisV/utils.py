...
import builder
from models.JRG_ASS_FisV import run_jrg
...
# generic helpers
# ========================================================
def _get_dataset_name(args=None, dataset_name=None):
    if dataset_name is not None:
        return dataset_name.lower()
    if args is not None and hasattr(args, "dataset_name"):
        return args.dataset_name.lower()
    return "aqa7"


def _is_fisv(args=None, dataset_name=None):
    return _get_dataset_name(args=args, dataset_name=dataset_name) == "fisv"
...
def feat_score_aug(feature1, feature_list, score1, score_list, aug_scale=0.3, count=None, with_weight=False):
    ...
    if not with_weight:
        ...
        aug_feat = feature1 + (
            r / torch.sum(weight_matrix, dim=1).unsqueeze(1).expand(-1, aug_vector.shape[1])
        ) * aug_vector
...
# ========================================================
# replay / exemplar helpers
# ========================================================
def after_train(t, task, exemplar_set, packed_list, args, jrg, score_rgs, seen_tasks):
    """
    AQA-7 packed sample: [whole, patch, score]
    Fis-V packed sample: [rgb, flow, skel, score]
    """
    m = args.memory_size / (t + 1)

    # score is always the last field
...
def get_m_exemplar_simple(task, packed_sorted_list, m, args, feature_extracor, rgs, seen_tasks):
    ...
    if args.replay_method == 'group_replay':
        ...
        loader = builder.load_data_from_list(
            packed_sorted_list,
            task,
            dataset_name=_get_dataset_name(args=args)
        )
        features2herding, scores2herding = get_features_score(
            feature_extracor, rgs, loader, seen_tasks, args
        )
...
def get_features_score(jrg, rgs, loader, seen_tasks, args):
    ...
    Support both:
    AQA-7 loader batch:
        (feat_whole, feat_patch, scores, action_id, data_id)

    Fis-V loader batch:
        (rgb, flow, skel, scores, action_id, data_id)
    """
    ...
    for batch_idx, batch in enumerate(loader):
        if dataset_name == 'fisv':
            rgb, flow, skel, scores, action_id, data_id = batch
            rgb = rgb.cuda()
            flow = flow.cuda()
            skel = skel.cuda()
            scores = scores.float()
            ...
            feat_whole, feat_patch, scores, action_id, data_id = batch
...
def reconstruct_exemplar_set(exemplar_set, dataset_name='aqa7'):
        ...
        if exemplar_set is not None:
            for a in range(len(exemplar_set)):
                ...
                for exemplar in exemplar_set[a]:
                    ...
                    train_score_with_memory = torch.cat(
                        (train_score_with_memory, torch.tensor(exemplar[3]).float().unsqueeze(0)), dim=0
                    )
    ...
    if exemplar_set is not None:
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                ...
                train_score_with_memory = torch.cat(
                    (train_score_with_memory, torch.tensor(exemplar[2]).float().unsqueeze(0)), dim=0
                )
                train_action_name_with_memory += [a]
...
def random_select_exemplar(combined_exemplar, b, count=0, dataset_name='aqa7'):
    dataset_name = dataset_name.lower()
    np.random.seed(count)

    if dataset_name == 'fisv':
        train_rgb_with_memory, train_flow_with_memory, train_skel_with_memory, \
            train_score_with_memory, train_action_name_with_memory = combined_exemplar

        select_id = np.random.randint(0, train_score_with_memory.shape[0], size=b)
        select_rgb = train_rgb_with_memory[select_id]
        select_flow = train_flow_with_memory[select_id]
        select_skel = train_skel_with_memory[select_id]
        select_score = train_score_with_memory[select_id]
        select_action_name = torch.Tensor(train_action_name_with_memory)[select_id]

        return select_rgb, select_flow, select_skel, select_score, select_action_name, select_id

    # default: AQA-7
    train_whole_with_memory, train_patch_with_memory, train_score_with_memory, \
        train_action_name_with_memory = combined_exemplar
