...
def _load_rg_joint_split(data_root, joint_root, action_name, split='train'):
    ...
    if not os.path.isdir(joint_root):
        raise FileNotFoundError(joint_root)

    vst_path = flow_path = label_path = joint_path = None
    for alias in ACTION_ALIASES[action_name]:
        ...
        try:
            if joint_path is None:
                joint_path = _find_file(joint_root, [alias, 'stgcn', split])
        except FileNotFoundError:
            pass

    if any(x is None for x in [vst_path, flow_path, label_path, joint_path]):
        raise FileNotFoundError(
            'Incomplete RG joint files for action={}, split={}: '
            'vst={}, flow={}, label={}, joint={}'.format(
                action_name, split, vst_path, flow_path, label_path, joint_path
            )
        )
...
def get_parser():
    ...
    parser.add_argument('--joint_root', type=str, default='./DATA/RG/stgcnske')
