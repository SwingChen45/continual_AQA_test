...
class ASS_JRG(nn.Module):
    def __init__(
        ...
        self.whole_fuse = nn.Sequential(
            nn.Linear(self.hidden1, self.hidden1 // 2),
    ...
    def forward(self, feat_vst, feat_flow, feat_joint):
        """
        feat_vst:   [B, T, 1024]
        feat_flow:  [B, T, 1024]
        ...
        whole_feat = self.whole_fuse(torch.cat([vst_whole, flow_whole], dim=-1))
        ...
        diffwhole_feat = self.diffwhole_fuse(torch.cat([vst_diff, flow_diff], dim=-1))
...
def run_jrg(model_, feat_vst, feat_flow, feat_joint, save_graph=False, seen_tasks=None, is_train=False, args=None):
    fused_feat, featmap_list = model_(feat_vst, feat_flow, feat_joint)
...
if __name__ == '__main__':
    net = ASS_JRG(whole_size=1024, patch_size=256, seg_num=72, joint_num=18, save_graph=True, G_E_graph=True)
    feat_vst = torch.randn(2, 72, 1024)
    feat_flow = torch.randn(2, 72, 1024)
    feat_joint = torch.randn(2, 72, 18, 256)
    fused_feat, featmaps = net(feat_vst, feat_flow, feat_joint)
