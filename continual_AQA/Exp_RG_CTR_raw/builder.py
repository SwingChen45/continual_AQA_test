...
def build_moodel(args):
    model_ = ASS_JRG(
        ...
        ctr_dynamic=args.ctr_dynamic,
        ctr_gamma=args.ctr_gamma,
        ctr_reduction=args.ctr_reduction,
    )
...
def _load_rg_joint_split(joint_root, action_name, split='train', seg_num=72):
    joint_root = _resolve_existing_path(joint_root)
    action_root = os.path.join(joint_root, action_name)
    if not os.path.isdir(action_root):
        raise FileNotFoundError(action_root)
    joint_dir = _find_average_dir(action_root, action_name)
    ...
    if any(x is None for x in [joint_path, label_path]):
        raise FileNotFoundError(
            'Incomplete RG raw skeleton files for action={}, split={}: joint={}, label={}'.format(
    ...
    if len(raw_joint) != n:
        raise ValueError(
            'Size mismatch in {}-{}: joint={}, labels={}'.format(
                action_name, split, len(raw_joint), n
            )
        )
    if raw_joint.ndim != 5:
...
def load_data(data_root, set_id, args, exemplar_set=None):
    ...
    train_pack = _load_rg_joint_split(args.joint_root, action_name, split='train', seg_num=args.seg_num)
    test_pack = _load_rg_joint_split(args.joint_root, action_name, split='test', seg_num=args.seg_num)
...
def get_parser():
    ...
    parser.add_argument('--g_e_graph', action='store_true', default=True)
    ...
    parser.add_argument('--ctr_dynamic', action='store_true', default=True)
    parser.add_argument('--disable_save_graph', action='store_true', default=False)
    parser.add_argument('--disable_g_e_graph', action='store_true', default=False)
    parser.add_argument('--disable_ctr_dynamic', action='store_true', default=False)
    ...
    parser.add_argument('--ctr_gamma', type=float, default=0.1)
    parser.add_argument('--ctr_reduction', type=int, default=8)
    ...
    parser.add_argument('--model_name', type=str, default='RG_CTR_RAW_ASS')
...
def check_args_valid(args):
    ...
    if args.disable_save_graph:
        args.save_graph = False
    if args.disable_g_e_graph:
        args.g_e_graph = False
    if args.disable_ctr_dynamic:
        args.ctr_dynamic = False
    return
