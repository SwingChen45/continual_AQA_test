...
def _temporal_resample_raw_skeleton(x, seg_num):
    # x: [N,C,T,V,M]
    n, c, t, v, m = x.shape
    if t == seg_num:
...
def _find_average_dir(action_root, action_name):
    candidates = []
    for name in os.listdir(action_root):
        full = os.path.join(action_root, name)
        if os.path.isdir(full) and 'average' in name.lower():
            candidates.append(full)
    if not candidates:
        raise FileNotFoundError('Cannot find *_average directory for {}'.format(action_name))
    return sorted(candidates)[0]


def _load_rg_joint_split(data_root, joint_root, action_name, split='train', seg_num=72):
    data_root = _resolve_existing_path(data_root)
    joint_root = _resolve_existing_path(joint_root)
    action_dir = os.path.join(data_root, action_name)
    joint_action_dir = os.path.join(joint_root, action_name)
    if not os.path.isdir(action_dir):
        raise FileNotFoundError(action_dir)
    if not os.path.isdir(joint_action_dir):
        raise FileNotFoundError(joint_action_dir)
    joint_dir = _find_average_dir(joint_action_dir, action_name)

    vst_path = flow_path = label_path = joint_path = joint_label_path = None
    for alias in ACTION_ALIASES[action_name]:
        try:
            if vst_path is None:
                vst_path = _find_file(action_dir, [alias, 'vst', split])
        except FileNotFoundError:
            pass
        try:
            if flow_path is None:
                flow_path = _find_file(action_dir, [alias, 'flow', split])
        except FileNotFoundError:
            pass
        try:
            if label_path is None:
                label_path = _find_file(action_dir, [alias, 'label', split])
        except FileNotFoundError:
            pass
        try:
            if joint_path is None:
                joint_path = _find_file(joint_dir, [alias, 'data', split])
        except FileNotFoundError:
            pass
        try:
            if joint_label_path is None:
                joint_label_path = _find_file(joint_dir, [alias, 'label', split])
        except FileNotFoundError:
            pass

    if any(x is None for x in [vst_path, flow_path, label_path, joint_path, joint_label_path]):
        raise FileNotFoundError(
            'Incomplete RG raw joint files for action={}, split={}: '
            'vst={}, flow={}, label={}, joint={}, joint_label={}'.format(
                action_name, split, vst_path, flow_path, label_path, joint_path, joint_label_path
            )
        )

    vst = np.load(vst_path).astype(np.float32)
    flow = np.load(flow_path).astype(np.float32)
    raw_joint = np.load(joint_path).astype(np.float32)
    sample_ids, scores = _load_label_pkl(label_path)
    joint_ids, joint_scores = _load_label_pkl(joint_label_path)

    n = len(scores)
    if not (len(vst) == len(flow) == len(raw_joint) == n):
        raise ValueError(
            'Size mismatch in {}-{}: vst={}, flow={}, joint={}, labels={}'.format(
                action_name, split, len(vst), len(flow), len(raw_joint), n
            )
        )
    if sample_ids != joint_ids:
        raise ValueError('Sample id mismatch between RG label and joint label for {}-{}'.format(action_name, split))
    if not np.allclose(scores, joint_scores):
        raise ValueError('Score mismatch between RG label and joint label for {}-{}'.format(action_name, split))
    if raw_joint.ndim != 5:
        raise ValueError('Expected raw skeleton [N,C,T,V,M], got {} from {}'.format(raw_joint.shape, joint_path))
    if raw_joint.shape[1] != 3 or raw_joint.shape[3] != 18:
        raise ValueError('Expected raw skeleton C=3,V=18, got {} from {}'.format(raw_joint.shape, joint_path))

    raw_joint = _temporal_resample_raw_skeleton(raw_joint, seg_num)
    joint = raw_joint[:, :, :, :, 0].transpose(0, 2, 3, 1).astype(np.float32)  # [N,T,V,3]
...
def load_data(data_root, set_id, args, exemplar_set=None):
    ...
    train_pack = _load_rg_joint_split(data_root, args.joint_root, action_name, split='train', seg_num=args.seg_num)
    test_pack = _load_rg_joint_split(data_root, args.joint_root, action_name, split='test', seg_num=args.seg_num)

    train_vst = train_pack['vst']
    train_flow = train_pack['flow']
    train_joint = train_pack['joint']
    train_scores = train_pack['scores'].copy()
    test_vst = test_pack['vst']
    test_flow = test_pack['flow']
    ...
    train_vst_with_memory = torch.tensor(train_vst).float()
    ...
    if (exemplar_set is not None) and (args.dataset_mixup):
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                train_vst_with_memory = torch.cat((train_vst_with_memory, torch.tensor(exemplar[0]).float().unsqueeze(0)), dim=0)
                train_flow_with_memory = torch.cat((train_flow_with_memory, torch.tensor(exemplar[1]).float().unsqueeze(0)), dim=0)
                train_joint_with_memory = torch.cat((train_joint_with_memory, torch.tensor(exemplar[2]).float().unsqueeze(0)), dim=0)
                train_scores_with_memory = torch.cat((train_scores_with_memory, torch.tensor(exemplar[3]).float().unsqueeze(0)), dim=0)
                train_action_name_with_memory += [a]

    g = torch.Generator()
    g.manual_seed(args.seed)
    dataset_train = Data.TensorDataset(
        train_vst_with_memory,
    ...
    loader_test = Data.DataLoader(dataset=dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    return dataset_train, dataset_test, loader_train, loader_test, (train_vst, train_flow, train_joint, train_scores)


def load_data_from_list(data_list=None, cls_label=0):
    train_vst = torch.Tensor([])
    train_flow = torch.Tensor([])
    ...
    if data_list is not None:
        for idx, exemplar in enumerate(data_list):
            train_vst = torch.cat((train_vst, torch.tensor(exemplar[0]).float().unsqueeze(0)), dim=0)
            train_flow = torch.cat((train_flow, torch.tensor(exemplar[1]).float().unsqueeze(0)), dim=0)
            train_joint = torch.cat((train_joint, torch.tensor(exemplar[2]).float().unsqueeze(0)), dim=0)
            train_scores = torch.cat((train_scores, torch.tensor(exemplar[3]).float().unsqueeze(0)), dim=0)
...
def get_parser():
    ...
    parser.add_argument('--data_root', type=str, default='./DATA/RG')
    parser.add_argument('--joint_root', type=str, default='./DATA/RG')
    ...
    parser.add_argument('--whole-size', type=int, default=1024)
    parser.add_argument('--patch-size', type=int, default=3)
    parser.add_argument('--task-num', type=int, default=4)
    parser.add_argument('--model_name', type=str, default='RG_RAW_JOINT_ASS')
