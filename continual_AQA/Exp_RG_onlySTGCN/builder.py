...
def _load_rg_one_split(data_root, action_name, split='train'):
    ...
    if any(x is None for x in [stgcn_path, label_path]):
        raise FileNotFoundError(
            f'Incomplete files for action={action_name}, split={split}. '
            f'Looked inside: {action_dir}. '
            f'Found: stgcn={stgcn_path}, label={label_path}'
        )
...
def load_data(data_root, set_id, args, exemplar_set=None):
    ...
    if (exemplar_set is not None) and (args.dataset_mixup):
        ...
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                train_stgcn_with_memory = torch.cat((train_stgcn_with_memory, torch.tensor(exemplar[0]).float().unsqueeze(0)), dim=0)
    ...
    purely_task_data = (train_stgcn, train_scores)
...
def load_data_from_list(data_list=None, cls_label=0):
    ...
    if data_list is not None:
        for idx, exemplar in enumerate(data_list):
            train_stgcn = torch.cat((train_stgcn, torch.tensor(exemplar[0]).float().unsqueeze(0)), dim=0)
...
def get_parser():
    ...
    parser.add_argument('--model_name', type=str, default='RG_onlySTGCN_ASS')
