...
Instead, RG's ST-GCN stream is projected into latent graph tokens, and
AGSG-style general/specific graphs are applied on these latent tokens.
"""
...
class ASS_JRG(nn.Module):
    def __init__(
        ...
        # ST-GCN-only stream encoders.
        self.stgcn_encoder = self.build_encoder(self.patch_size)
        self.stgcn_diff_encoder = self.build_encoder(self.patch_size)
        ...
        # Build latent graph tokens from the ST-GCN-only representation.
        self.token_projector = nn.Sequential(
            nn.Linear(self.token_dim, self.joint_num * self.token_dim),
            nn.Dropout(self.dropout_rate),
            nn.ReLU(True),
        )
    ...
    def _aggregate_pairwise_diff(self, token_bjtd, graphs, jcws, temporal=False):
        # token_bjtd: [B, J, T, D]
        x = token_bjtd.permute(0, 2, 3, 1)  # [B, T, D, J]
        if temporal:
            x_next = torch.cat([x[:, 1:], x[:, -1:].clone()], dim=1)
    ...
    def forward(self, feat_stgcn):
        ...
        """
        B, T, _ = feat_stgcn.shape

        # ST-GCN-only stream encoding.
        stgcn_whole = self.stgcn_encoder(feat_stgcn)
        whole_feat = self.whole_fuse(stgcn_whole)  # [B, T, token_dim]

        stgcn_diff = self.stgcn_diff_encoder(self._temporal_diff(feat_stgcn))
        diffwhole_feat = self.diffwhole_fuse(stgcn_diff)  # [B, T, token_dim]

        # Build latent graph tokens from the ST-GCN-only representation.
        token_seed = stgcn_whole
        ...
        token_bjtd = latent_tokens.permute(0, 2, 1, 3)
        token_btjd = token_bjtd.permute(0, 2, 1, 3)
...
def run_jrg(model_, feat_stgcn, save_graph=False, seen_tasks=None, is_train=False, args=None):
    if seen_tasks is None:
        seen_tasks = []
    fused_feat, featmap_list = model_(feat_stgcn)  # [B, A, 512]
...
if __name__ == '__main__':
    os_env = {}
    net = ASS_JRG(patch_size=256, seg_num=72, joint_num=17, out_dim=1, save_graph=True, G_E_graph=True, alpha=0.5, task_num=4)
    feat_stgcn = torch.randn(2, 72, 256)
    fused_feat, featmaps = net(feat_stgcn)
