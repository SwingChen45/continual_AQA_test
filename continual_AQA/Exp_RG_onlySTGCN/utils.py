...
def get_features_score(jrg, rgs, loader, seen_tasks, args):
    ...
    for batch_idx, (feat_stgcn, scores, action_id, data_id) in enumerate(loader):
        feat_stgcn = feat_stgcn.cuda()
        scores = scores.float()
        action_id = action_id.cuda()

        feat, _ = run_jrg(jrg, feat_stgcn, save_graph=args.save_graph, seen_tasks=seen_tasks, args=args)
        if args.save_graph:
    ...
    if exemplar_set is not None:
        for a in range(len(exemplar_set)):
            ...
            for exemplar in exemplar_set[a]:
                ...
                train_score = torch.cat((train_score, torch.tensor(exemplar[1]).float().unsqueeze(0)), dim=0)
                train_action_name += [a]
    if len(train_action_name) == 0:
        return None
    return train_stgcn, train_score, train_action_name



def random_select_exemplar(combined_exemplar, b, count=0):
    train_stgcn, train_score, train_action_name = combined_exemplar
    np.random.seed(count)
    ...
    return select_stgcn, select_score, select_action_name, select_id
